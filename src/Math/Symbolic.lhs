...
>evalS :: (Floating a, Eq a, Show a) => [(Expr a, a)] -> Expr a -> a
>evalS _ (Atom a)     = a
>evalS env (Sum a b)  = evalS env a + evalS env b
>evalS env (Prod a b) = evalS env a * evalS env b
>evalS env (Neg a)    = -(evalS env a)
>evalS env (Rec a)    = recip (evalS env a)
>evalS env s@(Symbol _) = fromMaybe (error ("No "++ show s ++" in env"))
>                                   (lookup s env)
>evalS env (Exp E b)  = exp (evalS env b)
>evalS env (E)        = exp 1
>evalS env (Exp a b)  = evalS env a ** evalS env b
>evalS env (Sin a)    = sin (evalS env a)
>evalS env (Cos a)    = cos (evalS env a)
>evalS env (Log a)    = log (evalS env a)
